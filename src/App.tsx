...
const AdminRoute = ({ children }: { children: React.ReactNode }) => {
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-stone-50"><p>Loading...</p></div>;
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 p-6 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full border border-stone-100">
          <h2 className="text-2xl font-serif text-slate-900 mb-4">Access Denied</h2>
          <p className="text-slate-600">You are signed in, but you are not authorized as an admin.</p>
        </div>
      </div>
    );
  }
