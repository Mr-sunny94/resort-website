...
export default function Home() {
  ...
  useEffect(() => {
    async function loadPublicData() {
      ...
      if (settingsRes.data) {
        ...
        const localMapEmbed = localStorage.getItem('map_embed_url');
        ...
        if (localMapEmbed) loadedSettings.map_embed_url = localMapEmbed;
      ...
      // Add a slight aesthetic delay before removing the loader
      setTimeout(() => {
  ...
  // Smart Map URL parser to handle raw iframes, standard links, and fallbacks
  const getMapUrl = () => {
    if (!settings) return null;
    
    const customUrl = settings.map_embed_url;
    if (customUrl) {
      // 1. If user pasted the whole <iframe> HTML code
      if (customUrl.includes('<iframe') && customUrl.includes('src=')) {
        const match = customUrl.match(/src="([^"]+)"/);
        if (match && match[1]) return match[1];
      }
      
      // 2. If user pasted a proper embed URL
      if (customUrl.includes('/embed') || customUrl.includes('output=embed')) {
        return customUrl;
      }
    }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-cyan-500 selection:text-slate-950 relative overflow-x-hidden transition-colors duration-300">
      ...
      <div className={cn(
        "fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 transition-all duration-1000 ease-in-out",
        ...
        <div className="flex flex-col items-center">
           ...
           <h1 className="text-3xl md:text-4xl font-syne font-bold tracking-[0.2em] uppercase text-slate-900 dark:text-white mb-4 drop-shadow-md">
             {settings?.resort_name || 'AURA HAVEN'}
           </h1>
           <span className="text-[10px] font-mono tracking-[0.3em] text-cyan-600 dark:text-cyan-400 uppercase flex items-center space-x-3">
      ...
      <section className="relative min-h-screen w-full flex items-center justify-center pt-28 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center mt-8">
          <h1 className="text-5xl sm:text-7xl md:text-8xl font-syne font-extrabold text-slate-900 dark:text-white mb-10 tracking-tight leading-none drop-shadow-[0_10px_25px_rgba(255,255,255,0.8)] dark:drop-shadow-[0_10px_25px_rgba(0,0,0,0.9)] transition-colors duration-300">
            FIND YOUR <br />
      ...
      {mapEmbedSrc && (
        <section id="location" className="py-28 px-6 relative z-10 border-t border-slate-200 dark:border-slate-800/60 bg-slate-50 dark:bg-slate-950">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
              <div>
                <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-slate-200/50 dark:bg-slate-900 border border-slate-300/50 dark:border-slate-800 mb-4">
                  <MapPin className="w-3.5 h-3.5 text-cyan-600 dark:text-cyan-400" />
                  <span className="text-xs font-mono uppercase tracking-widest text-cyan-700 dark:text-cyan-300">OUR LOCATION</span>
                </div>
                <h2 className="text-4xl md:text-5xl font-syne font-bold text-slate-900 dark:text-white">Find Us</h2>
              </div>
              <p className="text-slate-600 dark:text-slate-400 text-sm max-w-md font-light">
                {settings.resort_address}
              </p>
            </div>
            
            <div className="w-full h-[500px] rounded-3xl overflow-hidden glass-card shadow-2xl relative border border-slate-200 dark:border-slate-800">
              <iframe 
                src={mapEmbedSrc} 
                width="100%" 
                height="100%" 
                ...
                className="filter dark:invert-[90%] dark:hue-rotate-180 dark:contrast-80 transition-all duration-300"
              ></iframe>
            </div>
          </div>
        </section>
      ...
      <section id="amenities" className="py-28 px-6 relative z-10 border-t border-slate-200 dark:border-slate-800/60">
        <div className="max-w-7xl mx-auto">
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h2 className="text-4xl md:text-5xl font-syne font-bold text-slate-900 dark:text-white mb-2">Curated Resort Amenities</h2>
      ...
      <footer className="bg-slate-100 dark:bg-slate-950 border-t border-slate-200 dark:border-slate-900 py-16 px-6 relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div className="flex items-center space-x-3">
            {settings?.resort_logo_url ? (
              <img src={settings.resort_logo_url} alt={settings.resort_name} className="h-10 w-auto object-contain filter drop-shadow-[0_0_10px_rgba(6,182,212,0.3)] dark:drop-shadow-[0_0_10px_rgba(6,182,212,0.5)]" />
            ) : (
